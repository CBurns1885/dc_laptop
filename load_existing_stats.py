...
class StatsGenerator:
    ...
    def calculate_referee_stats(self):
        ...
        if 'Referee' not in self.df.columns:
            print(" No Referee column found, skipping")
            return
...
class StatsLoader:
    ...
    def _load_stats(self):
        ...
        if team_file.exists():
            ...
            print(f" Team stats not found: {team_file}")
        ...
        if ref_file.exists():
            ...
            print(f" Referee stats not found: {ref_file}")
    ...
    def get_team_features(self, team_name: str, venue: str) -> Dict[str, float]:
        ...
        if venue == 'home':
            ...
            return {
                'away_stat_win_rate': stats.get('away_win_rate', 0.27),
                'away_stat_goals_pg': stats.get('away_goals_per_game', 1.1),
                'away_stat_goals_against': stats.get('away_goals_against_per_game', 1.4),
                'away_stat_advantage': -stats.get('home_advantage', 0.0)
            }
    
    def get_referee_features(self, referee_name: Optional[str]) -> Dict[str, float]:
        """Get referee features"""
        if not referee_name or referee_name not in self.referee_stats:
            return {
                'ref_cards_per_match': 4.0,
                'ref_yellows_per_match': 3.5,
                'ref_home_bias': 0.0
            }
        
        stats = self.referee_stats[referee_name]
        return {
            'ref_cards_per_match': stats.get('cards_per_match', 4.0),
            'ref_yellows_per_match': stats.get('yellows_per_match', 3.5),
            'ref_home_bias': stats.get('home_bias', 0.0)
        }
    
    def add_to_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        """Add stats features to a dataframe"""
        df = df.copy()
        
        for idx, row in df.iterrows():
            # Home team features
            home_features = self.get_team_features(row['HomeTeam'], 'home')
            for key, val in home_features.items():
                df.at[idx, key] = val
            
            # Away team features
            away_features = self.get_team_features(row['AwayTeam'], 'away')
            for key, val in away_features.items():
                df.at[idx, key] = val
            
            # Referee features
            ref_features = self.get_referee_features(row.get('Referee'))
            for key, val in ref_features.items():
                df.at[idx, key] = val
