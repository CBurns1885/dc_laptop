...
class AccumulatorFinder:
    ...
    def find_high_confidence_bets(self, min_prob: float = 0.70) -> List[Dict]:
        ...
        for idx, row in self.df.iterrows():
            ...
            # Try BLEND first, fall back to DC, then P_ columns
            markets = {
                '1X2_H': (['BLEND_1X2_H', 'DC_1X2_H', 'P_1X2_H'], 'Home Win', 2.0),
                '1X2_D': (['BLEND_1X2_D', 'DC_1X2_D', 'P_1X2_D'], 'Draw', 3.5),
                '1X2_A': (['BLEND_1X2_A', 'DC_1X2_A', 'P_1X2_A'], 'Away Win', 2.5),
                'BTTS_Y': (['BLEND_BTTS_Y', 'DC_BTTS_Y', 'P_BTTS_Y'], 'BTTS Yes', 1.9),
                'BTTS_N': (['BLEND_BTTS_N', 'DC_BTTS_N', 'P_BTTS_N'], 'BTTS No', 1.8),
                #'OU_1_5_O': (['BLEND_OU_1_5_O', 'DC_OU_1_5_O', 'P_OU_1_5_O'], 'Over 1.5', 1.25),
                #'OU_1_5_U': (['BLEND_OU_1_5_U', 'DC_OU_1_5_U', 'P_OU_1_5_U'], 'Under 1.5', 3.5),
                #'OU_3_5_U': (['BLEND_OU_3_5_U', 'DC_OU_3_5_U', 'P_OU_3_5_U'], 'Under 3.5', 1.35),
                #'OU_4_5_U': (['BLEND_OU_4_5_U', 'DC_OU_4_5_U', 'P_OU_4_5_U'], 'Under 4.5', 1.15),

                }

            for market_key, (col_candidates, market_desc, realistic_odds) in markets.items():
                # Try to find first available column
                col_name = None
    ...
    def build_accumulators(self, min_legs: int = 3, max_legs: int = 6,
        ...
        for num_legs in range(min_legs, min(max_legs + 1, len(high_conf_bets) + 1)):
            for combo in combinations(high_conf_bets, num_legs):
                ...
                # Sort legs by date and league for better readability
                legs.sort(key=lambda x: (x.get('date', ''), x.get('league', '')))
